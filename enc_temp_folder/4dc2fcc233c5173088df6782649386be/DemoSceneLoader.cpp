...
void dae::DemoSceneLoader::EventExercice(Scene& scene)
{
	...
	auto stick = ControllerInput::ControllerButtons::LeftThumbstick;
	float stickSpeed{ 40.f };
	auto pMoveCommandStick = std::make_shared<dae::MoveStickCommand>(player1.get(), stickSpeed, controller->GetDirectionLeftThumbStick());

	controller->AttachCommandToThumbStick(pMoveCommandStick, stick);

	auto button = std::make_pair(ControllerInput::ControllerButtons::ButtonB, ControllerInput::ButtonState::Pressed);
	auto scoreCommand = std::make_shared<dae::ScoreCommand>(player1.get(), true);
	controller->AttachCommandToButton(scoreCommand, button);
	button = std::make_pair(ControllerInput::ControllerButtons::ButtonX, ControllerInput::ButtonState::Pressed);
	controller->AttachCommandToButton(scoreCommand, button);
	...
	scorePlayer1->GetTransform()->SetPosition(glm::vec2{ 10,325 });
